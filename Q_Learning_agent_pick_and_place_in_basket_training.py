...
# SETUP FOR TRAINING Q-LEARNING AGENT
...
if __name__ == "__main__":
    ...
    game_state = recv_socket_data(sock_game)
    ...
    for i in range(training_time):
        print("Start training episode: ", i)
        ...
        print("go for item: ", player.shopping_quant[0], item)
        ...
        # Compute path to next item for the Q learning agent
        ...
        # Start q-learning to pick and place the item in the basket from current player state
        ...
        while not game_state['gameOver']:
            ...
            action_index = player.choose_action(game_state)
            ...
            # if basket not picked up, continue to next epoch
            if not next_state['observation']['baskets']:
                print("Missed basket pickup")
                break

            # if toggle basket, assume basket is dropped
            if action_index == 5 or not holding_basket:
                holding_basket = False
                not_holding_basket_cnt += 1
            ...
            # if player is not holding the basket for 100 epochs, training is not useful, continue to next epoch 
            if not_holding_basket_cnt > 100:
                print("Lost basket for too long")
                break

            # print("Next state: ", next_state['observation'])

            # Define the reward based on the state and next_state
            reward = player.calculate_reward(game_state, next_state)  # You need to define this function
            print("------------------------------------------")
            print(reward, action_commands[action_index])
            print("------------------------------------------")
            # Update Q-table
            player.learning(action_index, reward, game_state, next_state)
            ...
            basket_contents = game_state['observation']['baskets'][0]['contents']
            basket_quant = game_state['observation']['baskets'][0]['contents_quant']
