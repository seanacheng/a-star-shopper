...
def calculate_reward(previous_state, current_state):
    ...
    carrot_shelf_position = [11.5, 17.5]
    previous_agent_position = previous_state['observation']['players'][0]['position']
    current_agent_position = current_state['observation']['players'][0]['position']
    previous_distance = euclidean_distance(previous_agent_position, carrot_shelf_position)
    current_distance = euclidean_distance(current_agent_position, carrot_shelf_position)
    previous_holding_food = previous_state['observation']['players'][0]['holding_food']
    current_holding_food = current_state['observation']['players'][0]['holding_food']
    previous_direction = previous_state['observation']['players'][0]['direction']
    current_direction = current_state['observation']['players'][0]['direction']

    if current_holding_food is None and current_distance < previous_distance:  # goal 1
        # no penalty for navigating to carrot shelf (without holding food)
        return 0
    elif previous_holding_food is None and current_holding_food == 'carrot':  # goal 2
        # small reward for picking up carrot
        return 1
    elif current_holding_food == 'carrot' and previous_direction == current_direction and previous_agent_position == current_agent_position:  # goal 3
        # big reward for holding on to carrot and stopping
        return 10
    else:
